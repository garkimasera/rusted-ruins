...
pub fn add_town(gd: &mut GameData, rid: RegionId, pos: Vec2d, town_id: &str) {
    let site_gen: &SiteGenObject = gobj::get_by_id(town_id);
    let town = Town::new(town_id);
    let mut site = Site::new(town_id, site_gen.map_template_id.len() as u32);
    ...
    for map_template_id in &site_gen.map_template_id {
